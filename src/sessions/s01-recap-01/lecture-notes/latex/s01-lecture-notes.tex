...
A program place data (numbers and characters) in it, and asks Java to perform
...
\begin{itemize}
  ...
  \item Lines 1 to 16 are the declaration of the class.

  \item Lines 2 to 7 are the definition of the main method, the entry point of
    the program.

  \item Lines 9 to 16 are the definition of another method.

  \item Lines 3 and 4 are for processing the input arguments into something
    useful (Strings to ints)

  \item Line 6 shows the output to the user.

  \item Line 5 is the one that actually does the work (finding the max).
...
\begin{itemize}
  ...
  \item ``Composite'' Types: (Arrays and Objects) String, Person, int[]\ldots
...
String a = "hello";
s.ToUppercas();
// a is a reference to an String object
// .ToUppercase is a method to turn all caracters in the string to uppercase
\end{blackboard}
...
The memory has not been initialized, so Java will not let you use that
variable:
...
$ javac Bla.java
/tmp/Bla.java:4: error: variable a might not have been initialized
        System.out.println(a);
                           ^
1 error
\end{blackboard}
...
\begin{itemize}
  ...
  \item \verb+short+ is 16 bits (-32\_768 ($-2^{15}$) - 32\_769 ($2^{15}-1$)), less than 33 thousands.
...
\begin{itemize}
  ...
  \item Literals:
    ...
    \begin{itemize}
      ...
      \item by unicode code: \verb+'\u8636'+ (kanji for tea)
...
\end{blackboard}
    ...
    This is way floats and doubles are not used for important operations like currency, the class BigDecimal is used instead (session 03).

  \item Printing: using the \verb+java.io.PrintStream.println(float f)+ or the \verb+java.io.PrintStream.println(double d)+ method.
